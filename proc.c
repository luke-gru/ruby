...
rb_sym_to_proc(VALUE sym)
{
    ...
    if ((ruby_single_main_ractor || rb_ractor_main_p_()) && !sym_proc_cache) {
    ...
    if (sym_proc_cache && (ruby_single_main_ractor || rb_ractor_main_p_()) && RARRAY_AREF(sym_proc_cache, index) == sym) {
        return RARRAY_AREF(sym_proc_cache, index + 1);
    }
    else {
        proc = sym_proc_new(rb_cProc, ID2SYM(id));
        if (ruby_single_main_ractor || rb_ractor_main_p_()) {
