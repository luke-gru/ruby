...
// Checks the current thread for ruby interrupts and runs the cleanup function `cf_func` with `cf_data` if `rb_ec_check_ints` is going to raise
// (before actually raising). See the `rb_threadptr_execute_interrupts` for info on when it can raise.
static void
